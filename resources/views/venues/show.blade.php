    ...
    <div class="grid gap-8 lg:grid-cols-12">
        ...
        <div class="lg:col-span-8 xl:col-span-8">
            ...
            <div x-show="activeTab === 'rules'" x-cloak class="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm sm:p-8 space-y-8">
                ...
                <div>
                    <h4 class="flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-rose-600 mb-4">
                    ...
                    @if($venue->cancellationPolicies->count() > 0)
                        <div class="space-y-3">
                            @foreach($venue->cancellationPolicies as $policy)
                            <div class="flex items-center justify-between border-b border-stone-100 pb-3 last:border-0 last:pb-0">
                                <span class="text-sm font-medium text-stone-600">Khách hủy trước <span class="font-bold text-zinc-900">{{ $policy->hours_before }} tiếng</span></span>
                                <span class="text-sm font-bold text-rose-600 bg-rose-50 px-2 py-1 rounded">Phạt {{ $policy->fee_percent }}%</span>
                            </div>
                            @endforeach
                            <div class="mt-4 rounded-xl bg-emerald-50 p-4 text-sm font-semibold text-emerald-700 border border-emerald-100 flex items-center justify-center gap-2">
                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>
                                Hủy sớm hơn các mốc trên: Miễn phí hoàn toàn (Hoàn 100%)
                            </div>
                        </div>
                    @else
                        <div class="rounded-xl bg-emerald-50 p-5 text-center border border-emerald-100">
                            <p class="text-sm font-bold text-emerald-700">Cơ sở này đang cho phép hủy sân hoàn toàn miễn phí (Hoàn 100% tiền).</p>
                        </div>
                    @endif
                </div>

                <div class="pt-6 border-t border-stone-100">
                     <h4 class="flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-stone-500 mb-4">
                     ...
                     <ul class="space-y-2.5 text-sm text-zinc-600">
                         @if(!empty($venue->rules))
                             @foreach(explode("\n", str_replace("\r", "", $venue->rules)) as $rule)
                                 @if(trim($rule) !== '')
                                     <li class="flex items-start gap-2">
                                         <span class="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-stone-300"></span>
                                         {{ trim($rule) }}
                                     </li>
                                 @endif
                             @endforeach
                         @else
                             <li class="flex items-start gap-2 italic text-stone-400">Chưa có nội quy cụ thể cho cơ sở này.</li>
                         @endif
                     </ul>
                </div>
            </div>
