...
class OwnerBookingRescheduleController extends Controller
{
    ...
    public function reject(Request $request, BookingRescheduleRequest $rescheduleRequest): RedirectResponse
    {
        ...
        $rescheduleRequest->update(['status'=>'rejected','owner_note'=>$data['owner_note'],'reviewed_by'=>$request->user()->id,'reviewed_at'=>now()]);
        try {
            app(\App\Services\NotificationService::class)->notifyCustomerRescheduleRejected($rescheduleRequest->booking);
        } catch (\Throwable $e) {
            // ignore
        }
