        ...
        <form method="POST" action="{{ route('owner.web.vouchers.update', $voucher->id) }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="number" name="max_uses_per_user" value="{{ old('max_uses_per_user', $voucher->max_uses_per_user) }}" placeholder="Mỗi khách được dùng tối đa bao nhiêu lần? VD: 1" min="1" class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm text-slate-800 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500">
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach([1 => 'Thứ 2', 2 => 'Thứ 3', 3 => 'Thứ 4', 4 => 'Thứ 5', 5 => 'Thứ 6', 6 => 'Thứ 7', 0 => 'Chủ Nhật'] as $val => $label)
                                @php
                                    $checked = false;
                                    if (is_array(old('apply_days'))) {
                                        $checked = in_array((string)$val, old('apply_days'));
                                    } elseif ($voucher->apply_days) {
                                        $checked = in_array($val, $voucher->apply_days) || in_array((string)$val, $voucher->apply_days);
                                    }
                                @endphp
                                <label class="flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-1.5 hover:bg-slate-50 cursor-pointer text-xs font-semibold">
                                    <input type="checkbox" name="apply_days[]" value="{{ $val }}" {{ $checked ? 'checked' : '' }} class="h-3.5 w-3.5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500">
                                    <span>{{ $label }}</span>
                                </label>
                            @endforeach
                        </div>
                    ...
                    <div>
                        ...
                        <div id="time-slots-container" class="space-y-2">
                            @php
                                $timeSlots = old('time_slots', $voucher->time_slots ?? []);
                                if (empty($timeSlots)) {
                                    $timeSlots = [['start' => '', 'end' => '']];
                                }
                            @endphp
                            @foreach($timeSlots as $index => $slot)
                                <div class="flex items-center gap-2 time-slot-row">
                                    <input type="time" name="time_slots[{{ $index }}][start]" value="{{ $slot['start'] ?? '' }}" class="rounded-xl border border-slate-200 px-3 py-1.5 text-xs focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500">
                                    <span class="text-xs text-slate-400">đến</span>
                                    <input type="time" name="time_slots[{{ $index }}][end]" value="{{ $slot['end'] ?? '' }}" class="rounded-xl border border-slate-200 px-3 py-1.5 text-xs focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500">
                                    @if($loop->first)
