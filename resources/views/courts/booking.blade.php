    ...
    <div class="flex items-center justify-between gap-4">
        ...
        <div class="hidden sm:block border-l border-stone-200/60 pl-6 pr-4">
            ...
            <select id="paymentMethod" class="rounded-lg border border-stone-200 bg-stone-50 px-2 py-1.5 text-sm font-medium text-zinc-900 outline-none">
                ...
                <option value="wallet">Số dư ví ({{ number_format(Auth::user()?->getOrCreateWallet()?->balance ?? 0) }}₫)</option>
            </select>
        </div>
...
<div id="lockReasonModal" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
    <div class="w-full max-w-sm rounded-2xl bg-white shadow-2xl overflow-hidden">
        <div class="border-b border-stone-100 bg-stone-50/50 p-4 flex items-center justify-between">
            <h3 class="text-base font-bold text-zinc-900">Thông báo từ Chủ sân</h3>
            <button onclick="document.getElementById('lockReasonModal').classList.add('hidden')" class="text-stone-400 hover:text-stone-600"><svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
        </div>
        <div class="p-6 text-center">
            <p id="lockReasonText" class="text-sm text-rose-600 bg-rose-50 p-3 rounded-lg border border-rose-100"></p>
        </div>
    </div>
</div>
@endsection
...
<script>
    ...
    datePicker.addEventListener('change', (e) => {
        ...
        // LOGIC MỚI: Cập nhật thanh địa chỉ URL mà không cần tải lại trang
    ...
    function fetchAvailability(dateString) {
        ...
        // THÊM CACHE BUSTER: Chống trình duyệt lưu dữ liệu cũ
        const timestamp = new Date().getTime();
        const url = `/api/courts/${courtId}/availability?date=${dateString}&_t=${timestamp}`;
    ...
    function createSlotCard(slot) {
        ...
        // VÁ LỖI BẢO MẬT: Bổ sung replace(/'/g, "%27") để tránh lỗi khi chữ có dấu nháy đơn
        const slotDataStr = encodeURIComponent(JSON.stringify(slot)).replace(/'/g, "%27"); 
        ...
        if (!slot.is_available) {
            if (slot.is_past) {
                ...
                div.innerHTML = `
                    ...
                    <div class="flex items-center justify-between opacity-40 mt-0.5">
                        <p class="text-sm font-medium text-slate-500">${priceFmt}</p>
                        <span class="text-[9px] font-bold uppercase text-slate-500">Quá giờ</span>
                    </div>
                `;
            } else if (slot.is_locked_by_owner) {
                div.classList.add('border-slate-300', 'bg-slate-100/80', 'cursor-pointer', 'hover:border-slate-400', 'hover:shadow-sm', 'transition');
                let reason = slot.lock_reason ? slot.lock_reason : 'Sân tạm nghỉ bảo trì.';
                // VÁ LỖI: Encode lý do khóa
                let encodedReason = encodeURIComponent(reason).replace(/'/g, "%27");
                div.setAttribute('onclick', `showLockReason('${encodedReason}')`);
                
                div.innerHTML = `
                    <div class="flex items-center justify-between opacity-90">
                        <h4 class="text-xs font-medium text-slate-700 line-through decoration-slate-400">${startTime} - ${endTime}</h4>
                        <svg class="h-3 w-3 text-slate-600" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z" /></svg>
                    </div>
                    <div class="flex items-center justify-between opacity-90 mt-0.5">
                        <p class="text-sm font-medium text-slate-600">${priceFmt}</p>
                        <span class="text-[9px] font-bold uppercase text-slate-700 bg-white border border-slate-300 px-1 rounded">Bị khóa</span>
                    </div>
                `;
            } else {
                ...
                div.innerHTML = `
                    <div class="flex items-center justify-between opacity-90">
                        <h4 class="text-xs font-bold text-rose-700 line-through decoration-rose-300">${startTime} - ${endTime}</h4>
                        <svg class="h-3 w-3 text-rose-500" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z" /></svg>
                    </div>
                    <div class="flex items-center justify-between opacity-90 mt-0.5">
                        <p class="text-sm font-medium text-rose-600">${priceFmt}</p>
                        <span class="text-[9px] font-bold uppercase text-rose-700 bg-white border border-rose-200 px-1 rounded shadow-sm">Đã đặt</span>
                    </div>
                `;
            }
        } else {
    ...
    window.addEventListener('pageshow', function(event) {
        // event.persisted = true nghĩa là trang đang được lấy từ bộ nhớ đệm lịch sử
        if (event.persisted) {
            // Xóa rác: Bỏ các ca đã chọn trong nháp
            selectedSlots = [];
            updateSummaryUI();

            // Đọc lại URL để chắc chắn lịch đang hiển thị đúng ngày
            const urlParams = new URLSearchParams(window.location.search);
            const dateFromUrl = urlParams.get('date');
            
            if (dateFromUrl) {
                datePicker.value = dateFromUrl;
                updateDateDisplay(dateFromUrl);
            }

            // ÉP tải lại danh sách ca từ Server
