...
<main class="mx-auto max-w-7xl px-6 py-8 font-[Inter] text-slate-800">
    ...
    <div class="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900">
                Danh sách System Voucher
            </h1>
            <p class="mt-2 text-sm text-slate-500">
                Quản lý các mã giảm giá áp dụng trên toàn bộ hệ thống.
            </p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.vouchers.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-5 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-emerald-700">
    ...
    <div class="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <form method="GET" action="{{ route('admin.vouchers.index') }}" class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-5">
            <!-- Tìm kiếm code/tên -->
            <div>
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.vouchers.show', $voucher->id) }}" class="rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-700 hover:bg-emerald-50 hover:text-emerald-600 transition">
                                        Chi tiết
                                    </a>
                                    ...
                                    <a href="{{ route('admin.vouchers.edit', $voucher->id) }}" class="rounded-lg bg-amber-50 px-3 py-1.5 text-xs font-bold text-amber-700 hover:bg-amber-100 transition">
                                        Sửa
                                    </a>

                                    <form method="POST" action="{{ route('admin.vouchers.toggle-status', $voucher->id) }}" class="inline">
                                        @csrf
                                        @method('PATCH')
                                    ...
                                    @if($voucher->used_count == 0)
                                        <form method="POST" action="{{ route('admin.vouchers.destroy', $voucher->id) }}" onsubmit="return confirm('Bạn có chắc chắn muốn xóa voucher này?');" class="inline">
                                            @csrf
                                            @method('DELETE')
