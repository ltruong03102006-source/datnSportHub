...
<div class="form-card">
    <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="name" class="form-control" value="{{ old('name', $user->name) }}" placeholder="Nhập họ và tên..." required>
            @error('name')
        ...
        <div class="form-group">
            ...
            <input type="email" name="email" class="form-control" value="{{ old('email', $user->email) }}" placeholder="Nhập email đăng nhập..." required>
            @error('email')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <span class="text-muted-small">Chỉ nhập nếu bạn muốn đổi mật khẩu mới cho người dùng này (ít nhất 8 ký tự).</span>
            @error('password')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            <div class="form-group">
                ...
                <select name="role" class="form-select" required>
                    ...
                    <option value="owner" {{ old('role', $user->role) == 'owner' ? 'selected' : '' }}>Chủ sân (Owner)</option>
            ...
            <div class="form-group">
                ...
                <select name="status" class="form-select" required>
                    ...
                    <option value="inactive" {{ old('status', $user->status) == 'inactive' ? 'selected' : '' }}>Vô hiệu hóa</option>
